...
let
  ...
  discoverSubcommands = cmdDir:
    let
      ...
      getDescription = cmdName:
        let
          # Look for the main .go file (usually named after the command)
          goFile = "${cmdDir}/${cmdName}/${cmdName}.go";
          
          # Read the file if it exists
          content = if builtins.pathExists goFile
                    then builtins.readFile goFile
                    else "";
          
          # Extract comment with "Description:" pattern
          # This is a simple heuristic - could be improved
          lines = lib.splitString "\n" content;
          descriptionLines = builtins.filter (line: 
            lib.hasInfix "Description:" line
          ) lines;
          
          extractedDesc = if descriptionLines != []
                         then lib.removePrefix "// Description: " (builtins.head descriptionLines)
                         else null;
        in
          extractedDesc;
  ...
  generateUsageCommands = commands:
    let
      # Determine padding for alignment
      maxNameLen = lib.foldl' (max: cmd: 
        if lib.stringLength cmd.name > max 
        then lib.stringLength cmd.name 
        else max
      ) 0 commands;
      
      # Pad command name to align descriptions
      padName = name: 
        let len = lib.stringLength name;
            padding = lib.concatStrings (lib.genList (_: " ") (maxNameLen - len + 2));
        in name + padding;
      
      commandLine = cmd:
        let
          desc = if cmd.description != null 
                 then cmd.description 
...
in
{
  ...
  writeMainGo = cmdDir: pkgs.writeTextFile {
    name = "main.go";
